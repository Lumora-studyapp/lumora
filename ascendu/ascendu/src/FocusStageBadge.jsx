...
export default function FocusStageBadge({stage,event,disabled=false,color='#2D6A4F',outgoingImage,incomingImage,imageScale=1,onPlaybackChange}) {
  ...
  return <span ref={root} className="sg-stage-badge" style={{'--stage-color':color}}>
    ...
    {playing&&<span key={burst.id} className="sg-stage-burst" aria-hidden="true" data-stage-advancement={burst.id}
      ...
      <svg className="sg-stage-rings" viewBox="0 0 320 320"><g fill="none" stroke="#EBCB69" strokeWidth=".8">
      ...
      <svg className="sg-stage-front" viewBox="0 0 320 320">
        <defs>
          <linearGradient id={`${id}-gold`} x2="0" y2="1"><stop stopColor="#FFF4AA"/><stop offset=".42" stopColor="#F4D75B"/><stop offset="1" stopColor="#B98222"/></linearGradient>
          <linearGradient id={`${id}-white`} x2="0" y2="1"><stop stopColor="#FFFEEE"/><stop offset="1" stopColor="#FFF79D"/></linearGradient>
          <radialGradient id={`${id}-glow`}><stop stopColor="#FFF9C0" stopOpacity=".48"/><stop offset="1" stopColor="#FFF9C0" stopOpacity="0"/></radialGradient>
          <pattern id={`${id}-dots`} width="7" height="7" patternUnits="userSpaceOnUse"><rect width="2.3" height="2.3" fill="#FFF5AB"/></pattern>
        ...
        {[0,1,2].map(n=><g key={n} className={`sg-stage-echo sg-stage-echo-${n}`}><use href={`#${id}-chevron`} fill="none" stroke="#DABA62" strokeWidth=".8"/></g>)}
        <g className="sg-stage-arrows"><use href={`#${id}-chevron`} fill={`url(#${id}-gold)`}/>
          <use className="sg-stage-white" href={`#${id}-chevron`} fill={`url(#${id}-white)`}/></g>
      </svg>
